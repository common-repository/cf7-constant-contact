...
.vx_check_item input{
  ...
  ?>
  <div class="vx_wrap"> 
    <h2 class="vx_img_head"><img alt="<?php esc_html_e("Constant Contact Feeds", 'contact-form-ccontact-crm') ?>" title="<?php esc_html_e("Constant Contact Feeds", 'contact-form-ccontact-crm') ?>" src="<?php echo $this->get_base_url()?>images/ccontact-crm-logo.png?ver=1" /> <?php 
echo sprintf(esc_html__('Edit Constant Contact Feed # %d','contact-form-ccontact-crm') ,$fid); 
  if(empty($id) || $new_account_id != $id){
 ?> <a href="<?php echo esc_url($new_feed_link) ?>" class="add-new-h2"><?php esc_html_e("Add New Feed",'contact-form-ccontact-crm'); ?></a> 
 <?php
}
 ?>
 <a href="<?php echo esc_url($feeds_link) ?>" class="add-new-h2"><?php esc_html_e("Back to Feeds",'contact-form-ccontact-crm'); ?></a>
...
if(empty($feed_name)){
  ...
  ?>
  <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(sprintf(esc_html__("We are unable to login to Constant Contact with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'contact-form-ccontact-crm'), "<a href='".esc_url($this->link_to_settings())."'>", "</a>")); ?></div>
  <?php
  // return;
  } 
  ?>
  ...
  <?php wp_nonce_field("vx_nonce") ?>
    <div class="vx_div">
      <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('1. Enter feed name',  'contact-form-ccontact-crm' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_name" class="left_header"><?php esc_html_e("Feed Name", 'contact-form-ccontact-crm'); ?>
  ...
  <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('2. Select Constant Contact Account.', 'contact-form-ccontact-crm'); ?></div>
<div class="crm_btn_div" title="<?php esc_html_e('Expand / Collapse','contact-form-ccontact-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_account" class="left_header"><?php esc_html_e("Constant Contact Account", 'contact-form-ccontact-crm'); ?>
  <?php $this->tooltip("vx_sel_object") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_account" class="crm_sel load_object" name="account" autocomplete="off">
  <option value=""><?php esc_html_e("Select a Constant Contact Account", 'contact-form-ccontact-crm'); ?></option>
  <?php
  ...
  <div class="clear"></div>
    ...
    <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('3. Select Contact Form.', 'contact-form-ccontact-crm'); ?></div>
<div class="crm_btn_div" title="<?php esc_html_e('Expand / Collapse','contact-form-ccontact-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_form_id" class="left_header"><?php esc_html_e("Contact Form Form", 'contact-form-ccontact-crm'); ?>
  <?php $this->tooltip("vx_sel_form") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_form_id" class="crm_sel load_object" name="form_id" autocomplete="off">
  <option value=""><?php esc_html_e("Select a Form", 'contact-form-ccontact-crm'); ?></option>
  <?php
...
<?php
  ...
  ?>  
<div class="howto"><?php echo sprintf(esc_html__('If contact form is missing from list then install our %s , submit required form, it will add that missing form in this list.','contact-form-ccontact-crm'),'<a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">Contact Form Entries</a>')?></div>
<?php
      }
?>
  </div>
  <div class="clear"></div>
    ...
    <div id="crm_ajax_div_object" style="display:none; text-align: center; line-height: 100px;"><i> <?php esc_html_e('Loading, Please Wait...','contact-form-ccontact-crm'); ?></i></div>
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php esc_html_e('This is a required field','contact-form-ccontact-crm') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  e.preventDefault(); 
    if(!confirm('<?php esc_html_e('Are you sure to remove ?','contact-form-ccontact-crm') ?>')){
      return;
  }
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php esc_html_e('Please Select Field Name','contact-form-ccontact-crm') ?>');
     return;   
    }  
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  $(document).on('click','.vxc_tips',function(e){
    ...
    jQuery('#crm_note_fields').select2({ placeholder: '<?php esc_html_e('Select Field','contact-form-ccontact-crm') ?>'});
} 
...
vx_select();
  function vx_select(){
      $('#vx_add_fields_select').select2({ placeholder: '<?php esc_html_e('Select Field','contact-form-ccontact-crm') ?>'});
      add_note_sel();
  }
