...
$this->push($entry,$form,'',false);    
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo esc_attr($id) ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Constant Contact Plugin','contact-form-ccontact-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','contact-form-ccontact-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo esc_attr($id) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Constant Contact Plugin','contact-form-ccontact-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
return $objects;
  ...
  */
  ...
  $desc= !empty($feed['error']) ? $feed['error'] : esc_html__("Unknown Error",'contact-form-ccontact-crm');
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.esc_attr($this->id).'_update" id="'.esc_attr($this->id).'_update" value="1" /><label for="'.esc_attr($this->id).'_update" title="%s">%s</label><br /><br />', esc_html__('Create or update this entry in Constant Contact. The fields will be mapped according to the form feed settings.', 'contact-form-ccontact-crm'), esc_html__('Send to Constant Contact', 'contact-form-ccontact-crm'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  $button.= '<input type="submit" class="button button-large button-secondary alignright" name="'.esc_attr($this->id).'_send" style="margin-left:5px; '.$margin.'" title="'.__('Create or update this entry in Constant Contact. The fields will be mapped according to the form feed settings.','contact-form-ccontact-crm').'" value="'.__('Send to Constant Contact', 'contact-form-ccontact-crm').'" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Constant Contact Logs','contact-form-ccontact-crm').'" href="'.esc_url($log_url).'">'.__('Constant Contact Logs','contact-form-ccontact-crm').'</a>';
...
$temp=$entry['__vx_data'];  
  ...
  if(!empty($matches[0])){
   ...
   foreach($matches[0] as $m){
       $m=trim($m,'{}');
    $t_val=$this->verify_field_val($entry,$m); 
    if(is_array($t_val)){ $t_val=implode(' ',$t_val); }    
    $vals['{'.$m.'}']=$t_val;  
   }
...
//  } 
  ...
  $log_link=' <a href="'.esc_url($log_url).'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','contact-form-ccontact-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
