...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo esc_attr($this->id) ?>_send" value="yes"> <?php esc_html_e('Send to Constant Contact','contact-form-ccontact-crm') ?></label>
  </div>
  <?php  
}
    /**
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . esc_html__('Settings', 'contact-form-ccontact-crm') . '</a>' );
        }
        return $links;
   }
  /**
...
public function tooltips(){
     self::$tooltips = array(
 'vx_feed_name' =>  esc_html__('Enter feed name of your choice.', 'contact-form-ccontact-crm'),
  'vx_sel_object' => esc_html__('Select the Object to Create when a Form is Submitted.', 'contact-form-ccontact-crm'),
   'vx_sel_account' =>__('Select the Constant Contact account you would like to export entries to.', 'contact-form-ccontact-crm'),
  'vx_sel_form' => esc_html__('Select the Contact Form you would like to integrate with Constant Contact. Contacts generated by this form will be automatically added to your Constant Contact account.', 'contact-form-ccontact-crm'),
  
  'vx_map_fields' => esc_html__('Associate your Constant Contact fields to the appropriate Contact Form fields.', 'contact-form-ccontact-crm'),
  ...
  'vx_manual_export' => esc_html__('If you do not want all entries sent to Constant Contact, but only specific, approved entries, check this box. To manually send an entry to Constant Contact, go to Entries, choose the entry you would like to send to Constant Contact, and then click the "Send to Constant Contact" button.', 'contact-form-ccontact-crm'),
  
    'vx_entry_notes' => esc_html__('Enable this option if you want to synchronize Contact Form entry notes to Constant Contact Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Constant Contact Object selected in the feed.', 'contact-form-ccontact-crm'),
    
      'vx_primary_key' => esc_html__('Which field should be used to update existing objects?', 'contact-form-ccontact-crm'),
...
<div class="table_head_i"><?php esc_html_e('Capsule', 'contact-form-ccontact-crm'); ?></div>  
<div class="vx_group">
...
if(isset($res['error'])){
  ...
  */
  ...
  $(this).text('<?php esc_html_e('Show Key','contact-form-ccontact-crm') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php esc_html_e('Hide Key','contact-form-ccontact-crm') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  esc_html_e('You do not have permissions to access this page','contact-form-ccontact-crm');    
  return;
  }
...
//
  ...
  $allowed=array('first_name','last_name','email_address','email','permission_to_send');
         if(!self::$is_pr){
...
public function groups($meta,$info){
 ...
 if(!empty($info['lists'])){ 
  ...
  <div class="vx_row">
   <div class="vx_col1">
  <label for="crm_sel_camp"><?php echo esc_html__('Select List(s)','contact-form-ccontact-crm') ?></label>
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = esc_html__('Constant Contact','contact-form-ccontact-crm');
        return $plugins;
    }
  /**
...
$field_options="<option>".esc_html__("Select Object",'contact-form-ccontact-crm')."</option>"; 
  if(is_array($objects)){
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  esc_html_e('You do not have permissions to save settings','contact-form-ccontact-crm');
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-ccontact-crm'));
...
if(isset($_POST[$this->id."_submit"])){ 
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  esc_html_e('You do not have permissions to edit/save feed','contact-form-ccontact-crm'); 
  return;
  }
  //
...
$fields['_vx_url']=array('id'=>'_vx_url','label'=>__('Page URL','contact-form-ccontact-crm')); 
$fields['_vx_created']=array('id'=>'_vx_created','label'=>__('Entry Created','contact-form-ccontact-crm')); 
$fields['_vx_updated']=array('id'=>'_vx_updated','label'=>__('Entry Updated','contact-form-ccontact-crm')); 
